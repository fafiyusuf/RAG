document.addEventListener("DOMContentLoaded", () => {
  ...
  feedbackForm.addEventListener("submit", async (e) => {
    ...
    if (!message) {
      ...
      feedbackStatus.className = "text-sm text-red-400 text-center mt-2";
      return;
    }

    submitFeedbackBtn.disabled = true;
    feedbackStatus.textContent = "Submitting...";
    feedbackStatus.className = "text-sm text-gray-400 text-center mt-2";
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        feedbackStatus.className = "text-sm text-green-400 text-center mt-2";
  ...
  // Attach a delegated click listener for the feedback button so it works
  // even if the element is added/modified after this script runs.
  document.body.addEventListener("click", (e) => {
    const btn = e.target.closest && e.target.closest("#feedbackBtn");
    if (!btn) return;
    // Debug helper - uncomment if you need to trace clicks in console
    // console.log('feedbackBtn clicked', e);
    ...
    if (!feedbackModalEl) return; // nothing to do

    // Reset modal state and open
    try {
      if (feedbackMessageEl) feedbackMessageEl.value = "";
      if (feedbackRatingEl) feedbackRatingEl.value = "";
      if (feedbackStatusEl) feedbackStatusEl.textContent = "";
      // reset selectedRating via existing variable and UI
      selectedRating = 0;
      updateStars();
    } catch (err) {
      // safe-noop
